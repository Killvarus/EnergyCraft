...
def motionBlur(image, kernelX, kernelY, kernel):
    """
    Эта функция фильтрует изображение image, вычисляя цвет каждого пикселя последовательно,
    делая усреднение по всем пикселям, которые соответствуют ненулевым элементам ядра kernel
    """
    # Делаем копию исходного изображения, в неё будем записывать изменённые пиксели
    imageBlurred = image.copy()
    ...
    for j in range(image.shape[0]):
        for i in range(image.shape[1]):
            ...
            imageBlurred[j,i,0] = rBlurred
            imageBlurred[j,i,1] = gBlurred
            imageBlurred[j,i,2] = bBlurred
    return imageBlurred
...
# Запускаем процесс размытия
start = time()
blurredImage = motionBlur(tonedImage, kernelX, kernelY, kernel)
